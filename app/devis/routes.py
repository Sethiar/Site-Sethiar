...
def process_devis(id, action):
    ...
    try:
        ...
        if action == "valider":
            devis.waiting_request_validate(new_status="validée")
            mail_reply_devis_validate(devis.email, devis.prenom)
            flash("La demande de devis a été traitée et validée.", "success")
        elif action == "refuser":
            devis.waiting_request_refusal(new_status="refusée")
            mail_reply_devis_reject(devis.email, devis.prenom)
